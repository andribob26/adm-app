...
import {
  ...
  getAllProduk,
  resetResProduk,
} from "../../store/slices/produkSlice";
import CurrencyFormat from "react-currency-format";

const ModalEdit = ({ valAksi, token }) => {
  const [isEdit, setIsEdit] = useState(true);
  const dispatch = useDispatch();
  const resProduk = useSelector((state) => state.produkSlice.resProduk);
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(editProduk({ id: valAksi.id, data: values, token: token }));
    },
  });

  useEffect(() => {
    if (resProduk.success) {
      if (resProduk.type === "edit") {
        const modal = window.Modal.getInstance(document.querySelector("#edit"));
        modal.hide();
        formik.resetForm();
        dispatch(getAllProduk({ token: token }));
        dispatch(resetResProduk());
      }
    }
  }, [resProduk.success]);
